...
static const u16 kvaser_pciefd_id_table[] = {
    KVASER_PCIEFD_4HS_ID,         KVASER_PCIEFD_2HS_ID,          KVASER_PCIEFD_HS_ID,
    KVASER_PCIEFD_MINIPCIE_HS_ID, KVASER_PCIEFD_MINIPCIE_2HS_ID, 0,
};
...
static int kvaser_pciefd_spi_wait_loop(const struct kvaser_pciefd* pcie, u32 msk) {
    u32 res;
    int i;
...
static int kvaser_pciefd_spi_cmd(const struct kvaser_pciefd* pcie, const u8* tx, u32 tx_len, u8* rx, u32 rx_len) {
    assert(pcie);
    assert(tx);
    assert(rx);
...
static int kvaser_pciefd_cfg_read_and_verify(const struct kvaser_pciefd* pcie, struct kvaser_pciefd_cfg_img* img) {
    int offset = KVASER_PCIEFD_CFG_IMG_OFFSET;
    int res;
    u32 crc;
    u8* crc_buff;

    u8 cmd[] = {KVASER_PCIEFD_FLASH_READ_CMD, (u8)((offset >> 16) & 0xff), (u8)((offset >> 8) & 0xff),
                (u8)(offset & 0xff)};

    assert(pcie);
    assert(img);
    res = kvaser_pciefd_spi_cmd(pcie, cmd, ARRAY_SIZE(cmd), (u8*)img, KVASER_PCIEFD_CFG_IMG_SZ);
    if (res) {
        return res;
    }
    ...
    if (le32_to_cpu(img->version) != KVASER_PCIEFD_CFG_SYS_VER) {
        kv_print_err("Error: Config flash corrupted, version number is wrong\n");
        return -ENODEV;
    }
    ...
    if (le32_to_cpu(img->crc) != crc) {
        kv_print_err("Error: Stored CRC does not match flash image contents\n");
        return -EIO;
    }
...
static void kvaser_pciefd_cfg_read_params(struct kvaser_pciefd* pcie, struct kvaser_pciefd_cfg_img* img) {
    ...
    kv_print_dbg("EAN: " KV_FLASH_EAN_FRMT_STR "\n", ean[1] >> 12,
                 ((ean[1] & 0xfff) << 8) | ((((ean[0])) >> 24) & 0xff), (ean[0] >> 4) & 0xfffff, ean[1] & 0x0f);
...
static int kvaser_pciefd_read_cfg(struct kvaser_pciefd* pcie) {
    ...
    if (cmd[0] != KVASER_PCIEFD_FLASH_ID_EPCS16) {
        kv_print_err("Error: Flash id is 0x%x instead of expected EPCS16 (0x%x)\n", cmd[0],
...
static int kvaser_pciefd_scan(struct kvaser_devices* devices) {
    ...
    for (pic_dev = pacc->devices; pic_dev; pic_dev = pic_dev->next) {
        ...
        pci_fill_info(pic_dev, PCI_FILL_IDENT | PCI_FILL_CLASS | PCI_LOOKUP_PROGIF | PCI_FILL_BASES | PCI_FILL_SIZES);
        ...
        if (pic_dev->device_class != KVASER_PCIEFD_PCI_CLASS_CAN) {
            kv_print_err("Warning: Wrong device class: Expected 0x%04x, got 0x%04x, ignoring this device\n",
                         KVASER_PCIEFD_PCI_CLASS_CAN, pic_dev->device_class);
            continue;
        }

        /* Setup the path to the pci device */
        snprintf(sysfs_base_path, sizeof(sysfs_base_path), "/sys/bus/pci/devices/%04x:%02x:%02x.%d", pic_dev->domain,
                 pic_dev->bus, pic_dev->dev, pic_dev->func);
        ...
        kv_print_dbg("Device found: 0x%04x:0x%04x in %s\n", pic_dev->vendor_id, pic_dev->device_id, sysfs_base_path);
        ...
        kv_libpci_dev->base_addr = pic_dev->base_addr[0] & PCI_ADDR_MEM_MASK;
...
static int kvaser_pciefd_set_device_name(struct kvaser_device* device) {
    ...
    for (i = 0; i < len; i++) {
        if ((device->ean[0] == kvaser_pciefd_dev_name_list[i].ean[0]) &&
            ...
            strncpy(device->device_name, kvaser_pciefd_dev_name_list[i].name, sizeof(device->device_name));
...
static int kvaser_pciefd_open(struct kvaser_device* device) {
    ...
    base = mmap((void*)kv_libpci_dev->base_addr, kv_libpci_dev->size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);

    if (base == MAP_FAILED || base == NULL) {
        kv_print_err(
            "Error: mmap() failed for \"%s\": %m\n"
...
static int epcs_sector_erase(const struct kvaser_pciefd* pcie, uint32_t offset) {
    int ret;
    u8 cmd[] = {KVASER_PCIEFD_FLASH_SEC_ERASE_CMD, (u8)((offset >> 16) & 0xff), (u8)((offset >> 8) & 0xff),
                (u8)(offset & 0xff)};
...
static int epcs_write_buffer(const struct kvaser_pciefd* pcie, int offset, const uint8_t* src_addr, int length) {
    int res;
    u8 cmd[] = {KVASER_PCIEFD_FLASH_PP_CMD, (u8)((offset >> 16) & 0xff), (u8)((offset >> 8) & 0xff),
                (u8)(offset & 0xff)};
    ...
    res = kvaser_pciefd_spi_cmd_inner(pcie, cmd, ARRAY_SIZE(cmd), cmd, 0, KVASER_PCIEFD_FLASH_CMD_MERGE);
    if (res) {
...
static int alt_epcs_flash_memcmp(const struct kvaser_pciefd* pcie, const uint8_t* buf, uint32_t offset, size_t n) {
    /*
    ...
    while (n > 0) {
        ...
        u8 cmd[] = {KVASER_PCIEFD_FLASH_READ_CMD, (u8)((flash_offset >> 16) & 0xff), (u8)((flash_offset >> 8) & 0xff),
                    (u8)(flash_offset & 0xff)};

        res = kvaser_pciefd_spi_cmd(pcie, cmd, ARRAY_SIZE(cmd), chunk_buffer, current_chunk_size);
        if (res) {
            /*
        ...
        res = memcmp(&buf[buf_offset], chunk_buffer, current_chunk_size);
        if (res) {
            return res;
        }
 ...
 */
static int alt_epcs_flash_erase_block(const struct kvaser_pciefd* pcie, uint32_t block_offset) {
    int ret;
...
static int alt_epcs_flash_write_block(const struct kvaser_pciefd* pcie,
    ...
    while (length) {
        ...
        next_page_start = (data_offset + KVASER_PCIEFD_FLASH_PAGE_SZ) & ~(KVASER_PCIEFD_FLASH_PAGE_SZ - 1);
        length_of_current_write = min(length, next_page_start - data_offset);
        write_len = epcs_write_buffer(pcie, data_offset, &(data)[buffer_offset], length_of_current_write);
        if (write_len < 0) {
            return write_len;
        }
...
static int alt_epcs_flash_write(const struct kvaser_pciefd* pcie,
    ...
    if (offset + length > KVASER_PCIEFD_FLASH_SZ) {
        kv_print_err("Error: alt_epcs_flash_write() is outside flash size. offset=0x%x + len=0x%x > 0x%lx\n", offset,
                     length, KVASER_PCIEFD_FLASH_SZ);
        return 1;
    }

    for (i = 0; i < KVASER_PCIEFD_FLASH_BLOCK_CNT; i++) {
        if ((offset >= current_offset) && (offset < (current_offset + KVASER_PCIEFD_FLASH_BLOCK_SZ))) {
            uint32_t data_to_write;
            /*
             * Check if the contents of the block are different
             * from the data we wish to put there
             */
            data_to_write = current_offset + KVASER_PCIEFD_FLASH_BLOCK_SZ - offset;
            data_to_write = min(data_to_write, remaining_length);

            if (alt_epcs_flash_memcmp(pcie, src_addr, offset, data_to_write)) {
                ret = alt_epcs_flash_erase_block(pcie, current_offset);
                if (ret) {
                    break;
                }

                ret = alt_epcs_flash_write_block(pcie, current_offset, offset, src_addr, data_to_write);
                if (ret) {
                    break;
                }
            } else {
                kv_print_dbg("block 0x%05x is equal, no need to erase nor re-write\n", current_offset);
            }
...
static int kvaser_pciefd_flash_prog(const struct kvaser_pciefd* pcie,
    ...
    if (stat != 0) {
        kv_print_err("Error: kvaser_pciefd_flash_prog write failed: %d\n", stat);
        return FIRMWARE_ERROR_FLASH_FAILED;
    }

    if (kvaser_pciefd_cmp_flash_img(pcie, addr, image, len)) {
        kv_print_err("Error: kvaser_pciefd_flash_prog compare failed\n");
        return FIRMWARE_ERROR_FLASH_FAILED;
    }
...
static int kvaser_pciefd_cmp_img_ean(const struct kvaser_pciefd* pcie, HydraImgHeader* sysHeader) {
    ...
    switch (fp->tag) {
        case FIRMWARE_DOWNLOAD_STARTUP:
            if (pcie->flash_img == NULL) {
                pcie->flash_img = malloc(KVASER_PCIEFD_FLASH_MAX_IMAGE_SZ);
                if (pcie->flash_img == NULL) {
                    kv_print_err("Error: malloc(KVASER_PCIEFD_FLASH_MAX_IMAGE_SZ)\n");
                    return -ENOMEM;
                }
            }
        ...
        case FIRMWARE_DOWNLOAD_WRITE:
            if (pcie->flash_img && ((fp->x.data.address + fp->x.data.len) < KVASER_PCIEFD_FLASH_MAX_IMAGE_SZ)) {
                memcpy(pcie->flash_img + fp->x.data.address, fp->x.data.data, fp->x.data.len);
            } else {
        ...
        case FIRMWARE_DOWNLOAD_COMMIT: {
            ...
            if (sysHeader->hdCrc != crc32Calc(pcie->flash_img, sizeof(HydraImgHeader) - 4)) {
            ...
            if (kvaser_pciefd_cmp_img_ean(pcie, sysHeader) != 0) {
                kv_print_err("Error: image EAN does not match device\n");
            ...
            for (i = 0; i < numberOfImages; i++) {
                ...
                if (sysHeader->imgCrc == crc32Calc(pcie->flash_img + sizeof(HydraImgHeader), sysHeader->imgLength)) {
                    ImageContainerInfo* imgInfo;
                    HydraImgHeader* imgHeader;
                    uint8_t* startAddr;

                    imgInfo = (ImageContainerInfo*)(pcie->flash_img + sizeof(HydraImgHeader) + 4);
                    imgHeader = (HydraImgHeader*)(pcie->flash_img + imgInfo[i].imgStartAddr);
                    startAddr = pcie->flash_img + imgInfo[i].imgStartAddr + sizeof(HydraImgHeader);

                    kv_print_dbg("Sys image validated. (V.%08x)\n", sysHeader->version);
                    if (!dryrun) {
                        fp->status =
                            kvaser_pciefd_flash_prog(pcie, imgHeader->prgAddr, startAddr, imgHeader->imgLength);
                        kv_print_info("Flash completed, status %d\n", fp->status);
                    } else {
                        kvaser_pciefd_cmp_flash_img(pcie, imgHeader->prgAddr, startAddr, imgHeader->imgLength);
                        kv_print_info("Dry-run\n");
                    }
                } else {
                    kv_print_err("Error: Image crc check failed. (V.%08x)\n", sysHeader->version);
                }
            }
            break;
        }
